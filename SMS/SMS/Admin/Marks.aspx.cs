...
namespace SMS.Admin
{
    public partial class Marks : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = fn.Fetch(@"Select Row_Number() over(Order by (Select 1)) as [Sr.No], Exam.ExamId, Exam.ClassId, 
                                      Class.ClassName, Exam.SubjectId, Subject.SubjectName, Exam.RollNo,Exam.TotalMarks,Exam.OutOfMarks from Exam 
                                      inner join Class on Exam.ClassId = Class.ClassId inner join Subject on  Exam.SubjectId = Subject.SubjectId; ");
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string outOfMarks = txtOutMarks.Text.Trim();
                ...
                if(dttbl.Rows.Count > 0)
                {
                    ...
                    if (dt.Rows.Count == 0)
                    {
                        ...
                        fn.Query(query);
                        ScriptManager.RegisterStartupScript(this, GetType(), "PopUp", "marksInsert()", true);
                        ddlClass.SelectedIndex = 0;
                        ddlSubject.SelectedIndex = 0;
                        txtRoll.Text = string.Empty;
                        txtStudMarks.Text = string.Empty;
                        txtOutMarks.Text = string.Empty;
                        GetMarks();
                    }
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "PopUp", "markAlEx()", true);
                    }
                }
                else
                {
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                ScriptManager.RegisterStartupScript(this, GetType(), "PopUp", "mUpdate()", true);
                GridView1.EditIndex = -1;
                GetMarks();
            }
            catch (Exception ex)
